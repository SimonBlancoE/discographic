...
const setupCodexAuthCommand = `set -e
if [ ! -f /mnt/host-codex/auth.json ]; then
  echo "Codex ChatGPT auth requires ~/.codex/auth.json on the host. Run codex login first." >&2
  exit 1
fi
mkdir -p "$CODEX_HOME"
cp /mnt/host-codex/auth.json "$CODEX_HOME/auth.json"
if [ -f /mnt/host-codex/config.toml ]; then
  cp /mnt/host-codex/config.toml "$CODEX_HOME/config.toml"
fi`;
