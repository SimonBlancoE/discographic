...
import type { ImportPreviewResponse, SyncStatus } from '../lib/types';
import type { ImportSyncState } from '../../shared/contracts/syncStatus.js';

const POLL_MS = 2000;
const MAX_POLL_ERRORS = 3;

type ImportPhase = 'idle' | 'loading' | 'preview' | 'applying' | 'syncing' | 'result';
...
function ImportButton({ disabled = false }) {
  ...
  const [syncState, setSyncState] = useState<ImportSyncState | null>(null);
  const fileRef = useRef<HTMLInputElement | null>(null);
  const disposed = useRef(false);
  const pollTimer = useRef<ReturnType<typeof window.setTimeout> | null>(null);
  ...
  const poll = useCallback(async () => {
    ...
    try {
      ...
      setStatusError(t('collection.importStatusError', { error: getErrorMessage(err, t('client.networkError')) }));
    }
  }, [t]);
  ...
  async function handleFileSelect(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setError(getErrorMessage(err, t('client.networkError')));
      setPhase('idle');
    }
