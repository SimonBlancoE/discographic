...
type ExportQuality = keyof typeof QUALITY_PRESETS;

function CoverWall({ releases, filters: availableFilters }: { releases: WallRelease[]; filters: CollectionFilterOptions }) {
  const { t } = useI18n();
  const toast = useToast();
  const [size, setSize] = useState(110);
  const [filters, setFilters] = useState<CollectionFilters>(() => createCollectionFilters());
  ...
  async function handleExport() {
    ...
    try {
      ...
      toast.error(t('wall.posterError', { error: getErrorMessage(error, t('client.tapeteError')) }));
    } finally {
      setExporting(false);
