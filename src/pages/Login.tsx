...
function Login() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const message = getErrorMessage(error, t('login.error'));
      setError(message);
      toast.error(message, t('login.denied'));
    } finally {
      setSubmitting(false);
    }
  }
      ...
      <form onSubmit={handleSubmit} className="glass-panel w-full space-y-5 p-8">
        <div>
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm uppercase tracking-[0.35em] text-brand-200">{t('login.eyebrow')}</p>
          ...
          <h1 className="mt-3 font-display text-4xl text-white">{t('login.title')}</h1>
