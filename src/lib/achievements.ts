...
import type { AchievementSet, HiddenAchievement, TieredAchievement, Translate } from './types';

type AchievementLocale = 'es' | 'en';
...
function topCount(list: Array<{ count?: number } | null | undefined> = []): number {
...
function buildTier(progress: number, tiers: TierDefinition[], t: Translate): TierProgress {
...
function createTieredAchievement(
  { id, emoji, title, description, progress, tiers }: {
    id: string;
    emoji: string;
    title: string;
    description: string;
...
function createHiddenAchievement(
  { id, emoji, title, description, unlocked }: {
    id: string;
    emoji: string;
    title: string;
    description: string;
...
export function buildAchievements(stats: DashboardStats | null, t: Translate, locale: AchievementLocale = 'es'): AchievementSet {
  if (!stats) {
