...
import type { ReleaseListEntry, WallRelease } from '../../shared/contracts/release.js';

export const WALL_GRID_GAP = 16;
export const WALL_TITLE_PANEL_HEIGHT = 64;
const WALL_OVERSCAN_ROWS = 2;

type WallGridFilters = Partial<CollectionFilters>;
type WallGridRelease = Pick<WallRelease, 'artist' | 'title' | 'year' | 'genres' | 'styles' | 'formats' | 'labels'>;

export type WallGridMetrics = {
  columns: number;
  columnWidth: number;
  cardHeight: number;
  rowStride: number;
...
function pickListEntryName(entry: ReleaseListEntry): string {
  return typeof entry === 'string' ? entry : String(entry?.name || '');
}

export function filterWallReleases<T extends WallGridRelease>(releases: T[], filters: WallGridFilters = {}): T[] {
  ...
  return releases.filter((release) => {
    ...
    if (filters.decade) {
      ...
      if (!Number.isFinite(start) || year == null || !(year >= start && year < start + 10)) {
        return false;
      }
    }

    if (filters.format) {
      const formats = (release.formats || []).map(pickListEntryName);
    ...
    if (filters.label) {
      const labels = (release.labels || []).map(pickListEntryName);
...
export function getWallCardPosition(index: number, metrics: WallGridMetrics, gap = WALL_GRID_GAP): {
  top: number;
  left: number;
  width: number;
} {
