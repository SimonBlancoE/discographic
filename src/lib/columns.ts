...
export const DEFAULT_VISIBLE: ColumnId[] = COLUMNS.filter((c) => !c.defaultHidden).map((c) => c.id);
export const MANDATORY: ColumnId[] = COLUMNS.filter((c) => c.mandatory).map((c) => c.id);
